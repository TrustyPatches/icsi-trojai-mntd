...
def main():
    ...
    # TP: TRUSTY PATCH
    from bertner import CustomBERTModel
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            input_ids, attention_mask, labels = batch['input_ids'], batch['attention_mask'], batch['labels']  # TRUSTY PATCH
        ...
        # Early stopping
        converged = eval_metric['accuracy'] > 0.83
        if converged:
            print('Sufficient accuracy ({})'.format(eval_metric['accuracy']))
            break

    if not converged:
        print("Model with sufficient accuracy not found.")
    elif args.output_dir is not None:
        accelerator.wait_for_everyone()
        unwrapped_model = accelerator.unwrap_model(model)

        filename = os.path.join(args.output_dir, str(uuid.uuid4()) + '.pt')
